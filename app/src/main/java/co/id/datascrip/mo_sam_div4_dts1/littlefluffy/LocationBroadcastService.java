 ...
 * receiver.
 */
public class LocationBroadcastService extends Service implements GoogleApiClient.ConnectionCallbacks,
    ...
    Runnable mTask = new Runnable() {
        ...
        public void run() {
            ...
            if (lastLocationBroadcastTimestamp == lastLocationUpdateTimestamp) {
                ...
                if (LocationLibrary.showDebugOutput)
                    Log.d(LocationLibraryConstants.TAG, TAG + ": No new location update found since " + LocationInfo.formatTimestampForDebug(lastLocationUpdateTimestamp));
                ...
                if (LocationLibrary.showDebugOutput)
                    Log.d(LocationLibraryConstants.TAG, TAG + ": New location update found at " + LocationInfo.formatTimestampForDebug(lastLocationUpdateTimestamp));
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if (LocationLibrary.showDebugOutput)
            Log.d(LocationLibraryConstants.TAG, TAG + ": onStartCommand");
    ...
    public boolean forceLocationUpdate() {
        ...
        if (LocationLibraryConstants.SUPPORTS_GINGERBREAD) {
            if (GooglePlayServicesUtil.isGooglePlayServicesAvailable(getApplicationContext()) == ConnectionResult.SUCCESS) {
                if (LocationLibrary.showDebugOutput)
                    Log.d(LocationLibraryConstants.TAG, TAG + ": Force a single location update using Google GMS Location, as current location is beyond the oldest location permitted");
                mGoogleApiClient.connect(); // this will cause an onConnected() or onConnectionFailed() callback
                return true;
            } else {
                if (LocationLibrary.showDebugOutput)
                    Log.d(LocationLibraryConstants.TAG, TAG + ": Force a single location update using Android AOSP location, as current location is beyond the oldest location permitted");
                ...
                try {
                    ...
                    } else locationManager.requestSingleUpdate(criteria, oneshotReceiver);
                    if (LocationLibrary.showDebugOutput)
                        Log.d(LocationLibraryConstants.TAG, TAG + ": schedule timer to kill locationlistener in 30 seconds");
                    new Timer().schedule(new TimerTask() {
                        public void run() {
                            try {
                                if (LocationLibrary.showDebugOutput)
                                    Log.d(LocationLibraryConstants.TAG, TAG + ": remove updates after 30 seconds");
    ...
    public void onConnected(Bundle arg0) {
        if (LocationLibrary.showDebugOutput)
            Log.d(LocationLibraryConstants.TAG, TAG + ": onConnected (Google GMS Location)");
    ...
    public void onDisconnected() {
        if (LocationLibrary.showDebugOutput)
            Log.d(LocationLibraryConstants.TAG, TAG + ": onDisconnected (Google GMS Location)");
    ...
    public void onLocationChanged(Location location) {
        if (LocationLibrary.showDebugOutput)
            Log.d(LocationLibraryConstants.TAG, TAG + ": onLocationChanged (Google GMS Location)");
