...
public class SalesTotalFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        txSubtotal = (RobotoEditText) root.findViewById(R.id.stf_subtotal);
        txPotongan = (RobotoEditText) root.findViewById(R.id.stf_potongan);
        txDPP = (RobotoEditText) root.findViewById(R.id.stf_dpp);
        txPPN = (RobotoEditText) root.findViewById(R.id.stf_ppn);
        txTotal = (RobotoEditText) root.findViewById(R.id.stf_total);
        txNotes = (RobotoEditText) root.findViewById(R.id.stf_notes);
        btSimpan = (RobotoButton) root.findViewById(R.id.btSimpan);
        ...
        btSimpan.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (isValid()) {
                    ...
                    */
                    new Process_Sales_Header(getActivity()).Posting(Input_Order_Activity.kegiatan, new CallbackSalesHeader.CBPosting() {
                        @Override
                        public void onCB(Kegiatan kegiatan) {
                            if (kegiatan != null) {
                                Toast.makeText(getActivity(), "Order berhasil disimpan.", Toast.LENGTH_LONG).show();
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        ...
        menu.clear();
        String no = new SalesHeaderSQLite(getActivity()).get(Input_Order_Activity.kegiatan.getID()).getOrderNo();
        if (no == null)
            no = "";
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.save:
                if (isValid()) {
                    setHeader();
                    if (Input_Order_Activity.kegiatan.getSalesHeader().getHeaderID() >= Const.DEFAULT_HEADER_ID || Input_Order_Activity.kegiatan.getSalesHeader().getHeaderID() == 0) {
                        ...
                        new SalesHeaderSQLite(getActivity()).Post(Input_Order_Activity.kegiatan);
                        getActivity().finish();
                    } else
