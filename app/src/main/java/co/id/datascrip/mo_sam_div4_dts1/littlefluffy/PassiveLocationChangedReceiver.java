...
public class PassiveLocationChangedReceiver extends BroadcastReceiver {
    ...
    protected static void processLocation(final Context context, final Location location, final boolean batchResponses, final boolean forceBroadcast) {
        ...
        if (!usePreviousReading) {
            ...
            if (LocationLibrary.showDebugOutput)
                Log.d(LocationLibraryConstants.TAG, TAG + ": Storing location update, lat=" + thisLat + " long=" + thisLong + " accuracy=" + thisAccuracy + " time=" + LocationInfo.formatTimestampForDebug(thisTime));
        } else {
            if (LocationLibrary.showDebugOutput)
                Log.d(LocationLibraryConstants.TAG, TAG + ": Storing location update, less accurate so reusing prior location - time=" + LocationInfo.formatTimestampForDebug(thisTime));
        }
        ...
        if (thisTime - previousTime > LocationLibrary.getAlarmFrequency() || forceBroadcast) {
            ...
            if (LocationLibrary.showDebugOutput)
                Log.d(LocationLibraryConstants.TAG, TAG + ":processLocation: treating this location update as a periodic update, timestamp=" + LocationInfo.formatTimestampForDebug(thisTime));
            if (batchResponses) {
