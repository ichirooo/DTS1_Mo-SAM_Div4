...
import co.id.datascrip.mo_sam_div4_dts1.adapter.Adapter_List_Log;
...
public class PopUpComment extends PopupWindows implements PopupWindow.OnDismissListener {
        ...
        btReply.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (!txReply.getText().toString().trim().isEmpty()) {
                    ...
                    f.setBex(Global.getBEX(context));
                    f.setLineID(item.getID());
                    ...
                    */
                    new Process_Feedback(context, "Loading...").Send_Reply(f, new CallbackFeedback.CBPost() {
                        @Override
    ...
    private void refresh() {
        logs = new LogSQLite(context).get(item.getID());
