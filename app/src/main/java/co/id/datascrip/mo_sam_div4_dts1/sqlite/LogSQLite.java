...
public class LogSQLite {
    ...
    public boolean Insert(Log_Feed log) {
        try {
            ...
            values.put(ConstSQLite.KEY_LOG_EMPLOYEE_NO, log.getFeedback().getBex().getEmpNo());
        ...
        try {
            ...
            if (c.moveToFirst()) {
                do {
                    ...
                    log.setDate(c.getString(c.getColumnIndex(ConstSQLite.KEY_LOG_DATETIME)));

                    BEX bex = new BEX();
                    bex.setEmpNo(c.getString(c.getColumnIndex(ConstSQLite.KEY_LOG_EMPLOYEE_NO)));

                    feedback.setBex(bex);
                    feedback.setLineID(c.getInt(c.getColumnIndex(ConstSQLite.KEY_LOG_LINE_ID)));
                    feedback.getBex().setEmpNo(c.getString(c.getColumnIndex(ConstSQLite.KEY_LOG_EMPLOYEE_NO)));
