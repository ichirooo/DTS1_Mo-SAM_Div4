...
public class Retro {
    ...
    public static final String PACKAGE_NAME = "co.id.salamander.mo_sam_di4_dts1";
    ...
    public static String getString(Response<ResponseBody> response) {
        ...
        try {
            if (bodyResponse.isSuccessful()) {
                ...
                InputStream i = getErrorBody(bodyResponse).byteStream();
    ...
    public static RetroStatus getRetroStatus(Response<ResponseBody> response, String json) {
        ...
        if (Utils.isEmpty(json)) {
            ...
            try {
                ...
                if (jsonObject.has(RetroData.RETRO_STATUS))
                    retroStatus.setSuccess(jsonObject.getString(RetroData.RETRO_STATUS).trim().toLowerCase().equals(STATUS_SUCCESS));
                else if (jsonObject.has("retroStatus"))
                    retroStatus.setSuccess(jsonObject.getInt("retroStatus") == STATUS_SUCCESS_CODE);
                if (jsonObject.has(RetroData.RETRO_TITLE))
                    retroStatus.setMessage(jsonObject.getString(RetroData.RETRO_TITLE).trim());
                if (jsonObject.has(RetroData.RETRO_MESSAGE))
                    retroStatus.setMessage(jsonObject.getString(RetroData.RETRO_MESSAGE).trim());
                else if (jsonObject.has("message"))
                    retroStatus.setMessage(jsonObject.getString("message").trim());
                if (jsonObject.has(RetroData.RETRO_SQL))
                    retroStatus.setQuery(jsonObject.getString(RetroData.RETRO_SQL).trim());
