...
import com.salamander.mo_sam_div4_dts1.FileUtils;
 ...
 * getFromSalesHeader result in {@code onActivityResult} method of the same Activity or Fragment.
 *
...
public class QuickImagePick {
    ...
    private static final String PREFS_REQUEST_TYPE = "mosam_dts1_req_type";
    private static final String PREFS_LAST_CAMERA_URI = "mosam_dts1_last_cam_uri";
    private static final String PREFS_CAMERA_DIR = "mosam_dts1_cam_dir";
    private static final String PREFS_ALLOWED_MIME_TYPE_PRE_KITKAT = "mosam_dts1_allowed_mime_type_pre_kitkat";
    private static final String PREFS_ALLOWED_MIME_TYPES = "mosam_dts1_allowed_mime_types";
    private static final String PREFS_ALLOW_LOCAL_CONTENT_ONLY = "mosam_dts1_local_content_only";
    ...
    private static int ID_KEGIATAN = 0;
    ...
    public static void setNo(int id_kegiatan, int no) {
        NO = no;
    ...
    private static File createImageFile(@NonNull final Context pContext) {
        ...
        final String fileName = App.getUser(pContext).getInitial() + "_" + String.valueOf(ID_KEGIATAN) + "_" + String.valueOf(NO) + ".jpg";

        File file = new File(dir, fileName);
        if (file.exists())
            file.delete();
        current_file = new File(dir, fileName);
        return current_file;
    }
    ...
    private static Uri createImageUri(@NonNull final Context pContext, @NonNull final File pFile) {
        return FileProvider.getUriForFile(pContext, pContext.getPackageName() + ".mosam_dts1_file_provider", pFile);
        //return Uri.fromFile(pFile);
    }
    ...
    private static void handleResultFromGallery(@NonNull final Context pContext, final int pRequestType, @NonNull final Callback pCallback,
        ...
        current_file = new File(FileUtils.getRealPathFromURI(pContext, pictureUri));
        if (pictureUri == null) {
    ...
    /**
     * @param pContext app {@link Context}
     * @param pUri     uri to getFromSalesHeader MIME type for
    ...
    public static String getMimeType(@NonNull final Context pContext, @NonNull final Uri pUri) {
        ...
        // first try to getFromSalesHeader it from content resolver
    ...
    public static String getFileExtension(@NonNull final Context pContext, @NonNull final Uri pUri) {
        ...
        // first try to getFromSalesHeader extension from mime type
        ...
        if (mimeType != null) {
            ...
            // if content resolver fails then try to getFromSalesHeader it from url
