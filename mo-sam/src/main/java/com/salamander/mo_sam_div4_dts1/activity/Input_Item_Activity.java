...
import com.salamander.mo_sam_div4_dts1.adapter.Adapter_AutoComplete_Item;
import com.salamander.mo_sam_div4_dts1.adapter.Find_Item_Adapter;
import com.salamander.mo_sam_div4_dts1.custom.PopUpComment;
...
public class Input_Item_Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        txDescription = findViewById(R.id.iia_description);
        txPriceList = findViewById(R.id.iia_pricelist);
        txQty = findViewById(R.id.iia_qty);
        txDiscountPct = findViewById(R.id.iia_discount_pct);
        txDiscount = findViewById(R.id.iia_discount_value);
        txUnit = findViewById(R.id.iia_unit);
        txSubtotal = findViewById(R.id.iia_subtotal);
        btFind = findViewById(R.id.iia_btFind);
        btOK = findViewById(R.id.iia_btOk);
        btCancel = findViewById(R.id.iia_btCancel);
        btShowLog = findViewById(R.id.iia_btShowReply);
        linear_logs = findViewById(R.id.iia_linear_log);
        txLastDate = findViewById(R.id.iia_last_date_log);
        txLastLog = findViewById(R.id.iia_last_text_log);
        txLastInputDate = findViewById(R.id.iia_last_update_price);
        ...
        txDescription.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                new Proses_Sales_Header(context).getPrice(item.getCode(), new Callbacks.OnCBGetPrice() {
                    @Override
                    public void onCB(RetroStatus retroStatus, double price) {
        ...
        btFind.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                new Proses_Sales_Header(context).findItem(txDescription.getText().toString().trim(), new Callbacks.OnCBFindItem() {
                    @Override
                    public void onCB(RetroStatus retroStatus, ArrayList<Item> list_item) {
                        if (retroStatus.isSuccess()) {
                            if (list_item.size() > 0)
                                showDialog(list_item, txDescription.getText().toString());
                            else
        ...
        btOK.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                item.setDiscountValue(h.getDouble(txDiscount));
                item.setDiscountPct(h.getDouble(txDiscountPct));
                item.setSubtotal(h.getDouble(txSubtotal));
                item.setDescription(txDescription.getText().toString());
    ...
    private void setData(Item item) {
        if (item != null) {
            ...
            txUnit.setText(item.getUnit());
            txPriceList.setText(h.formatNumber(item.getPrice()));
            txDiscount.setText(h.formatNumber(item.getDiscountValue()));
            txDiscountPct.setText(h.formatNumber(item.getDiscountPct()));
            txQty.setText(String.valueOf(item.getQuantity()));
            txSubtotal.setText(h.formatNumber(item.getSubtotal()));/*
            ArrayList<Log_Feed> logs = new LogSQLite(this).get(item.getIDServer());
            if (logs != null && logs.size() > 0) {
                for (int i = 0; i < logs.size(); i++) {
                    if (logs.get(i).getFeedback().getUser().getEmpNo() == App.getUser(context).getEmpNo())
                        continue;
                    else {
                        txLastDate.setText(logs.get(i).getLogDate().getTglString());
