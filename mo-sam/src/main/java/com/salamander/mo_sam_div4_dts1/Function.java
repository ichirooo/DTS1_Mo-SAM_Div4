...
import com.salamander.mo_sam_div4_dts1.custom.ExceptionDialog;
import com.salamander.mo_sam_div4_dts1.custom.TimeOutDialog;
import com.salamander.mo_sam_div4_dts1.custom.UnknownHostExceptionDialog;
...
public class Function {
    ...
    /*
        public void startPositionService(Kegiatan kegiatan) {
            ...
            intent.putExtra("bex", App.getSalesperson().getEmpNo());
            intent.putExtra("id_kegiatan", kegiatan.getIDServer());
            intent.setAction(App.getSalesperson().getInitial());
    ...
    */
    public static void sendLocation(Context context, int id_kegiatan) {
        ...
        double latitude = sessionManager.getLastLocation().getLatitude();
        double longitude = sessionManager.getLastLocation().getLongitude();

        if (latitude != 0 && longitude != 0) {
            /*
            new Proses_Send_Position(context).execute(String.valueOf(id_kegiatan),
                    String.valueOf(App.getSalesperson().getEmpNo()),
                    String.valueOf(latitude),
                    String.valueOf(longitude));
            *//*
            new Proses_Send_Position().Send_Position(context, String.valueOf(id_kegiatan),
                    String.valueOf(App.getUser(context).getInitial()),
                    String.valueOf(App.getUser(context).getEmpNo()),
                    String.valueOf(latitude),
                    String.valueOf(longitude));
                    */
